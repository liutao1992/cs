...
const state={phase:'menu',round:0,wins:0,losses:0,time:150,elapsed:0,weapon:0,weapons:[],reload:0,cooldown:0,recoil:0,flash:0,shots:0,hits:0,kills:0,headshots:0,aim:false,mouseDown:false,bomb:null,plant:0,botCount:0};
let bots=[],effects=[],roundDecals=[],toastTime=0,hitTime=0,damageTime=0,footstep=0,botThink=0,frameCounter=0,fpsTime=0,clockTime=0,hasFired=false,lastBombBeep=0;
...
function startRound(){
  ...
  Object.assign(state,{phase:'playing',time:150,elapsed:0,weapon:0,weapons:weaponDefinitions.map(d=>({...d,ammo:d.capacity})),reload:0,cooldown:0,recoil:0,shots:0,hits:0,kills:0,headshots:0,aim:false,mouseDown:false,bomb:null,plant:0});
  resetInput();botThink=0;footstep=0;damageTime=0;hitTime=0;lastBombBeep=0;$('damage').style.opacity='0';$('killfeed').replaceChildren();
  const level=levels[config.level];state.botCount=level.count;
  for(let i=0;i<level.count;i++){const rig=createBot(scene,i),spawn=enemySpawns[i];const bot={...rig,id:i,name:['Viper','Nomad','Falcon','Ghost','Raven','Echo','Wolf'][i],health:100,path:[],pathTime:0,fireTimer:level.reaction+Math.random(),reaction:0,revealed:0,seen:false,patrol:i,death:0};bot.group.position.set(spawn[0],0,spawn[1]);bot.hitboxes.forEach(m=>{m.userData.bot=bot;});bots.push(bot);}
  ...
  toast(config.mode==='demolition'?'前往 A / B 点，按住 E 安放炸弹':'清剿开始 · 消灭全部敌人');updateHUD();requestCapture();
}
...
function menu(){state.phase='menu';resetInput();document.exitPointerLock?.();['hud','pause','result','scope'].forEach(id=>setVisible(id,false));setVisible('menu',true);$('damage').style.opacity='0';}
function finish(win,reason){if(state.phase!=='playing')return;state.phase='result';resetInput();if(win)state.wins++;else state.losses++;$('result-label').textContent=win?'MISSION COMPLETE':'MISSION FAILED';$('result-title').textContent=win?(config.mode==='demolition'&&state.bomb?'目标已摧毁':'区域已肃清'):'行动未完成';$('result-description').textContent=reason;$('result-kills').textContent=state.kills;$('result-accuracy').textContent=accuracy();$('result-time').textContent=formatTime(state.elapsed);setVisible('result',true);setVisible('scope',false);document.exitPointerLock?.();audio.tone(win?660:130,.5,.25,'triangle');}
...
function shoot(){
  ...
  if(hit?.object.userData.bot){const b=hit.object.userData.bot;const head=!!hit.object.userData.head||hit.point.y>b.group.position.y+1.52;b.health-=w.damage*(head?4:1);b.revealed=3;b.reaction=2;state.hits++;hitTime=.15;$('hitmarker').style.color=head?'#f5ad80':'#fff';audio.hit();if(b.health<=0){state.kills++;if(head)state.headshots++;const row=document.createElement('div'),who=document.createElement('em'),enemy=document.createElement('span');who.textContent='你';enemy.textContent=b.name+(head?' · 爆头':'');row.append(who,document.createTextNode(`  ─ ${w.name} → `),enemy);$('killfeed').prepend(row);while($('killfeed').children.length>5)$('killfeed').lastChild.remove();if(state.kills===state.botCount&&!state.bomb)finish(true,'所有敌人已被消灭，行动区域安全。');}}
...
function updateBots(dt){const level=levels[config.level];botThink-=dt;const think=botThink<=0;if(think)botThink=.13;
  for(const b of bots){
    ...
    if(b.seen){b.reaction+=dt;b.revealed=1.2;b.group.rotation.y=Math.atan2(-dx,-dz);if(b.reaction>level.reaction&&b.fireTimer<=0){b.fireTimer=.38+Math.random()*.45;audio.shot(0,true);const from=pos.clone().add(new THREE.Vector3(0,1.3,0)),to=new THREE.Vector3(player.x,player.y+1.2,player.z);tracer(from,to,0xf3ac62);const chance=level.accuracy*(dist<12?1.15:.8)*(keys.has('KeyC')?.8:1);if(Math.random()<chance){const damage=applyDamage(player.health,player.armor,level.damage);Object.assign(player,damage);damageTime=.3;audio.noise(.08,500,.18);if(player.health<=0){finish(false,`你被 ${b.name} 击倒。利用掩体、短点射和静步重新组织进攻。`);break;}}}}
    ...
    if(!b.seen||dist>16){
      ...
      const target=b.path[0];if(target){const tx=target.x-pos.x,tz=target.z-pos.z,len=Math.hypot(tx,tz);if(len<.3)b.path.shift();else{moveWithCollision(pos,tx/len*level.speed*dt,tz/len*level.speed*dt,world.obstacles,.38);if(!b.seen)b.group.rotation.y=Math.atan2(-tx,-tz);b.limbs.forEach((leg,i)=>leg.rotation.x=Math.sin(clockTime*9+i*Math.PI)*.4);}}
...
function updatePlayer(dt){
  const crouch=keys.has('KeyC'),walk=keys.has('ShiftLeft')||keys.has('ShiftRight'),speed=(crouch?2.1:walk?2.6:4.8)*(state.aim?.68:1)*(state.plant>0?0:1);let mx=Number(keys.has('KeyD'))-Number(keys.has('KeyA')),mz=Number(keys.has('KeyS'))-Number(keys.has('KeyW'));const moving=mx!==0||mz!==0;
...
function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;clockTime+=dt;frameCounter++;fpsTime+=dt;if(fpsTime>.75){$('fps').textContent=`${Math.round(frameCounter/fpsTime)} FPS`;fpsTime=0;frameCounter=0;}
  ...
  if(state.phase==='playing'){state.time-=dt;state.elapsed+=dt;updatePlayer(dt);if(state.phase==='playing')updateBots(dt);if(state.phase==='playing')updateBomb(dt);if(state.time<=0&&!state.bomb&&state.phase==='playing')finish(false,'行动时间已耗尽。下次请加快推进。');updateEffects(dt);updateHUD();drawMap($('radar'),world,player,bots,state.bomb);}
...
document.querySelectorAll('[data-level]').forEach(button=>button.addEventListener('click',()=>{config.level=button.dataset.level;document.querySelectorAll('[data-level]').forEach(b=>{b.classList.toggle('selected',b===button);b.setAttribute('aria-pressed',String(b===button));});$('bot-count').textContent=levels[config.level].count;}));
