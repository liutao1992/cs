...
function frame(now){requestAnimationFrame(frame);const realDt=Math.min((now-last)/1000,.05);last=now;effects.setLowHealth(state.phase==='playing'&&player.health<=28);effects.update(realDt);state.timeScale=slowUntil>now?.28:Math.min(1,state.timeScale+realDt*1.4);document.body.classList.toggle('slowmo',state.timeScale<.9);const dt=realDt*state.timeScale;clockTime+=dt;frameCounter++;fpsTime+=dt;if(fpsTime>.75){$('fps').textContent=`${Math.round(frameCounter/fpsTime)} FPS`;fpsTime=0;frameCounter=0;}
