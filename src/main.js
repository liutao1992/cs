...
import { BOT_ROLES, roleForIndex, applyRole, chooseObjective } from './bot-ai.js';
import { rollUpgrades, applyUpgrade, upgradeMods, waveStartBonus } from './upgrades.js';
import { createMeta, FINISHES, applySkin } from './mastery.js';
import { MODIFIERS, resolveModifier, enemyModifiers } from './modifiers.js';
...
const knifeRig=new THREE.Group();weaponScene.add(knifeRig);{const blade=new THREE.Mesh(new THREE.BoxGeometry(.028,.012,.34),new THREE.MeshStandardMaterial({color:0xc9d2d4,metalness:.9,roughness:.25}));blade.position.set(0,0,-.18);const edge=new THREE.Mesh(new THREE.BoxGeometry(.034,.004,.3),new THREE.MeshStandardMaterial({color:0xe8eef0,metalness:.95,roughness:.15}));edge.position.set(0,-.006,-.19);const guard=new THREE.Mesh(new THREE.BoxGeometry(.07,.02,.03),new THREE.MeshStandardMaterial({color:0x2a2f2c,metalness:.5,roughness:.6}));guard.position.set(0,0,-.01);const handle=new THREE.Mesh(new THREE.BoxGeometry(.034,.045,.13),new THREE.MeshStandardMaterial({color:0x3a3a34,roughness:.9}));handle.position.set(0,-.015,.07);knifeRig.add(blade,edge,guard,handle);}knifeRig.position.set(.26,-.3,-.72);knifeRig.rotation.set(-.2,-.15,.35);knifeRig.visible=false;
...
function spawnBot(i,stats=null,elite=false){
  ...
  const base=levels[config.level],role=elite?'elite':roleForIndex(i,config.mode),roleConfig=BOT_ROLES[role]||BOT_ROLES.rifle,enemy=state.modifier?enemyModifiers(state.modifier):{accuracy:0,reaction:1,speed:1,damage:0};
  const rig=createBot(scene,i,elite,faction,roleConfig.weapon),spawns=config.mode==='competitive'&&state.side==='CT'?world.map.tBotSpawns:world.map.botSpawns,spawn=spawns[i%spawns.length],health=elite?600:(stats?.health??100);
  const bot={...rig,id:i,name:elite?'泰坦':botNames[i],health,maxHealth:health,path:[],pathTime:0,fireTimer:base.reaction+Math.random(),reaction:0,revealed:0,seen:false,patrol:i,death:0,elite,damageMult:elite?1.5:1,role,roleFire:roleConfig.fireDelay,roleSpread:roleConfig.fireSpread,retreatT:0,strafeT:0,strafeDir:1};
  bot.stats=elite?{accuracy:Math.min((stats?.accuracy??base.accuracy)+.1,.9),reaction:(stats?.reaction??base.reaction)*.8,speed:(stats?.speed??base.speed)*.85,damage:base.damage+6}:applyRole({accuracy:(stats?.accuracy??base.accuracy)+enemy.accuracy,reaction:(stats?.reaction??base.reaction)*enemy.reaction,speed:(stats?.speed??base.speed)*enemy.speed,damage:(stats?.damage??base.damage)+enemy.damage},role);
  bot.nadeRole=config.mode==='competitive'&&!elite&&i%3===1;
  if(config.mode==='competitive'&&!elite&&world.sites.length){const site=world.sites[i%world.sites.length];bot.anchor={x:site.x+((i%3)-1)*4,z:site.z+(((i>>1)%3)-1)*4};}
...
function spawnWave(n){
  ...
  const bonus=waveStartBonus(state,player);
  if(st.boss){audio.bossRoar();toast(`第 ${n} 波 · Boss「泰坦」登场${bonus.length?` · ${bonus.join(' / ')}`:''}`);}else toast(`第 ${n} 波来袭 · ${st.count} 名敌人${bonus.length?` · ${bonus.join(' / ')}`:''}`);
...
let upgradeOffer=null,upgradeTimer=0,upgradeWave=0,supplyOffer=null,supplyTimer=0;
function renderOffer(eyebrow,title,cards){
  $('upgrade-eyebrow').textContent=eyebrow;$('upgrade-title').textContent=title;
  $('upgrade-cards').replaceChildren(...cards.map((card,index)=>{const el=document.createElement('div');el.className='upgrade-card';el.style.borderColor=(card.tone||'#d5ed8b')+'66';el.innerHTML=`<b style="color:${card.tone||'#d5ed8b'}">${card.icon||'★'}</b><strong>${card.name}</strong><small>${card.desc}</small><kbd>${index+1}</kbd>`;return el;}));
  setVisible('upgrade-overlay',true);
}
function offerUpgrades(){if(upgradeOffer)return;upgradeOffer=rollUpgrades(state.upgrades);if(!upgradeOffer.length)return;upgradeTimer=8;renderOffer('WAVE CLEAR','选择强化',upgradeOffer);}
function chooseUpgrade(index){if(!upgradeOffer)return;const def=upgradeOffer[index]||upgradeOffer[0];applyUpgrade(def.id,{player,state},state.upgrades);audio.perk();toast(`强化 · ${def.name}`);upgradeOffer=null;setVisible('upgrade-overlay',false);updateHUD();}
function offerSupply(){if(supplyOffer||upgradeOffer||state.supplyUsed)return;state.supplyUsed=true;const pool=[...PERK_DEFS];supplyOffer=[];while(supplyOffer.length<3&&pool.length)supplyOffer.push(pool.splice(Math.floor(Math.random()*pool.length),1)[0]);supplyTimer=7;renderOffer('KILL STREAK','连杀奖励',supplyOffer);}
function applySupplyPerk(def){switch(def.type){case'health':player.health=Math.min(state.maxHealth||100,player.health+40);break;case'armor':player.armor=Math.min(100,player.armor+50);break;case'speed':state.buffs.speed=clockTime+10;break;case'damage':state.buffs.damage=clockTime+10;break;case'ammo':{const w=state.weapons[state.weapon],d=weaponDefinitions[state.weapon];if(d)w.reserve=d.reserve;break;}}}
function chooseSupply(index){if(!supplyOffer)return;const def=supplyOffer[index]||supplyOffer[0];applySupplyPerk(def);audio.perk();toast(`连杀奖励 · ${def.name}`);supplyOffer=null;setVisible('upgrade-overlay',false);}
function closeOffers(){upgradeOffer=null;supplyOffer=null;setVisible('upgrade-overlay',false);}
function updateUpgradeFlow(dt){
  if(upgradeOffer){upgradeTimer=Math.max(0,upgradeTimer-dt);$('upgrade-progress').style.width=`${upgradeTimer/8*100}%`;if(upgradeTimer<=0)chooseUpgrade(Math.floor(Math.random()*upgradeOffer.length));}
  if(supplyOffer){supplyTimer=Math.max(0,supplyTimer-dt);$('upgrade-progress').style.width=`${supplyTimer/7*100}%`;if(supplyTimer<=0)chooseSupply(Math.floor(Math.random()*supplyOffer.length));}
  if(config.mode==='survival'){
    if(survival.phase==='break'&&!upgradeOffer&&upgradeWave!==survival.wave){upgradeWave=survival.wave;offerUpgrades();}
    if(survival.phase!=='break'&&upgradeOffer)chooseUpgrade(Math.floor(Math.random()*upgradeOffer.length));
  }
}
function onKill(b,head,w){
  ...
  b.deathDir=head?-1:Math.random()<.5?1:-1;b.deathRoll=(Math.random()-.5)*.9;
  audio.death();effects.shake(.14);effects.sparks(b.group.position.clone().setY(1.2),head?14:8,head?0xffb0a0:0xa32b22);
  for(const other of bots){if(other===b||other.health<=0)continue;if(other.group.position.distanceTo(b.group.position)<14){other.suspect={x:player.x,z:player.z};other.revealed=Math.max(other.revealed,2.6);}}
  ...
  if(level>=4)slowmo();
  const mods=upgradeMods(state.upgrades||{});if(mods.lifesteal)player.health=Math.min(state.maxHealth||100,player.health+mods.lifesteal);
  const knife=/刀/.test(w.name),nade=/雷/.test(w.name);
  meta.registerKill(state.weapon,{head,knife,nade});
  meta.recordEvent('kill');if(head)meta.recordEvent('headshot');if(knife)meta.recordEvent('knife');if(nade)meta.recordEvent('nade');
  if(level>=2&&(config.mode==='elimination'||config.mode==='demolition'))offerSupply();
...
function clearEffects(){for(const fx of tracers){scene.remove(fx.mesh);fx.mesh.geometry.dispose();fx.mesh.material.dispose();}tracers=[];effects.clear();throwables.clear();for(const d of roundDecals){scene.remove(d);d.geometry.dispose();d.material.dispose();}roundDecals=[];}
...
function startRound(){
  ...
  perks.reset();slowUntil=0;state.timeScale=1;document.body.classList.remove('slowmo');closeOffers();upgradeWave=0;refreshSkins();
...
function menu(){state.phase='menu';closeOffers();resetInput();document.exitPointerLock?.();['hud','pause','result','scope'].forEach(id=>setVisible(id,false));setVisible('menu',true);$('damage').style.opacity='0';renderBest();renderHall();}
function finish(win,reason){if(state.phase!=='playing')return;state.phase='result';closeOffers();resetInput();if(win)state.wins++;else state.losses++;
  meta.recordEvent(win?'win':'loss');
  if(state.modifier?.scoreMult&&state.modifier.scoreMult!==1)state.score=Math.round(state.score*state.modifier.scoreMult);
...
function updateHUD(){const w=state.weapons[state.weapon];if(!w)return;$('health').textContent=Math.ceil(player.health);$('armor').textContent=Math.ceil(player.armor);$('health-bar').style.width=`${Math.min(100,player.health/(state.maxHealth||100)*100)}%`;$('health-bar').style.background=player.health<30?'#f08a70':'#d5ed8b';$('ammo').textContent=w.ammo;$('reserve').textContent=w.reserve;$('weapon-label').textContent=w.name;$('weapon-state').textContent=state.reload>0?'RELOADING…':w.description;$('timer').textContent=formatTime(state.bomb?state.bomb.time:state.time);$('timer').style.color=state.bomb?'#f5ad80':state.time<20?'#f5ad80':'';$('enemies-left').textContent=bots.filter(b=>b.health>0).length;$('objective-text').textContent=state.bomb?(config.mode==='competitive'&&state.side==='CT'?(state.bomb.plantedByBot?'炸弹已安放 · 按住 E 拆除':'阻止敌人安放'):'守卫目标，等待引爆'):config.mode==='demolition'?'在 A / B 点安放炸弹':config.mode==='competitive'?(state.side==='T'?'安放并守卫炸弹（E）':'阻止安放 · 必要时拆除炸弹'):config.mode==='survival'?'抵御无尽波次 · 拾取补给强化':'消灭全部敌人';
  ...
  const comp=config.mode==='competitive';$('money-hud').hidden=!comp;const hasNades=!!(state.nades&&(state.nades.he||state.nades.flash||state.nades.smoke));$('nade-slots').hidden=!comp&&!hasNades;
  [['he','nade-he'],['flash','nade-flash'],['smoke','nade-smoke']].forEach(([type,id])=>{const el=$(id);el.textContent=state.nades?.[type]??0;el.parentElement.classList.toggle('selected',state.nade===type);});
  if(comp){$('money').textContent=`$ ${economy.money}`;if(!$('buy-menu').hidden){if(!canBuy())toggleBuy(false);else{$('buy-time').textContent=Math.max(0,Math.ceil(BUY_TIME-state.elapsed));$('buy-money').textContent=`$ ${economy.money}`;}}}}
function switchWeapon(index){if(state.weapon===index||state.phase!=='playing'||state.melee>0)return;if(!state.weapons[index]?.owned){toast('未持有该武器 · 按 B 购买');return;}state.weapon=index;state.reload=0;state.cooldown=.24;state.aim=false;hasFired=state.mouseDown;guns.forEach((g,i)=>g.visible=i===index);audio.reload();updateHUD();}
...
function tracer(from,to,color=0xffdf91){const geo=new THREE.BufferGeometry().setFromPoints([from,to]),material=new THREE.LineBasicMaterial({color,transparent:true,opacity:.8});const line=new THREE.Line(geo,material);scene.add(line);tracers.push({mesh:line,life:.065,total:.065});}
...
function explodeBarrel(ref){
  if(!ref||ref.exploded)return;ref.exploded=true;
  if(ref.mesh)scene.remove(ref.mesh);
  const point=new THREE.Vector3(ref.x,1,ref.z);
  effects.explosion(point,1.15);audio.explosion();
  for(const bot of bots){if(bot.health<=0)continue;const distance=Math.hypot(bot.group.position.x-ref.x,bot.group.position.z-ref.z);const damage=blastDamage(distance,ref.damage,ref.radius);if(!damage)continue;bot.health-=damage;bot.revealed=3;bot.reaction=2;bot.suspect={x:player.x,z:player.z};if(bot.health<=0){state.kills++;onKill(bot,false,{name:'爆炸油桶'});}}
  const playerDistance=Math.hypot(player.x-ref.x,player.z-ref.z),playerDamage=blastDamage(playerDistance,ref.damage,ref.radius);
  if(playerDamage)damagePlayerFromNade(playerDamage,'爆炸的油桶');
  for(const other of world.explosives){if(other!==ref&&!other.exploded&&Math.hypot(other.x-ref.x,other.z-ref.z)<ref.radius)explodeBarrel(other);}
}
function cycleNade(){if(state.phase!=='playing')return;const order=['he','flash','smoke'],available=order.filter(type=>state.nades[type]>0);if(!available.length){audio.deny();toast('没有投掷物 · 竞技模式按 B 购买');return;}const index=available.indexOf(state.nade);state.nade=available[(index+1)%available.length];toast(`已选择 ${THROW_DEFS[state.nade].name}`);updateHUD();}
function throwSelected(){if(state.phase!=='playing')return;let type=state.nade;if(!type){type=['he','flash','smoke'].find(name=>state.nades[name]>0);if(!type){audio.deny();toast('没有投掷物 · 按 B 购买');return;}}state.nades[type]--;state.nade=null;camera.updateMatrixWorld(true);const direction=new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion);direction.y+=.13;direction.normalize();throwables.throwNade(type,camera.position.clone().addScaledVector(direction,.55),direction);audio.tone(280,.07,.09,'square');effects.shake(.06);toast(`已投出 ${THROW_DEFS[type].name}`);updateHUD();}
function melee(){if(state.phase!=='playing'||state.melee>0||state.reload>0)return;state.melee=.42;audio.knife();effects.shake(.07);camera.updateMatrixWorld(true);const forward=new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion);let target=null,best=3;for(const b of bots){if(b.health<=0)continue;const to=new THREE.Vector3(b.group.position.x,b.group.position.y+1.1,b.group.position.z).sub(camera.position);const distance=to.length();if(distance>2.4)continue;const facing=forward.dot(to.normalize());if(facing<.55)continue;if(distance<best){best=distance;target=b;}}if(!target)return;const b=target;let diff=Math.atan2(-(player.x-b.group.position.x),-(player.z-b.group.position.z))-b.group.rotation.y;diff=Math.atan2(Math.sin(diff),Math.cos(diff));const backstab=Math.abs(diff)>2.2;const damage=(backstab?220:58)*(state.buffs.damage>clockTime?2:1);b.health-=damage;b.revealed=4;b.reaction=2;state.hits++;hitTime=.18;effects.blood(b.group.position.clone().setY(1.2),forward);audio.knifeHit();if(b.health<=0){state.kills++;onKill(b,false,{name:backstab?'战术刀 · 背刺':'战术刀'});}}
function visibleToBot(bot,dist){const start=bot.group.position.clone().add(new THREE.Vector3(0,1.55,0));const target=new THREE.Vector3(player.x,player.y+(keys.has('KeyC')?1.05:1.58),player.z);direction.copy(target).sub(start);ray.set(start,direction.normalize());ray.far=dist-.3;if(ray.intersectObjects(world.solids,false).length)return false;return !throwables.smokeBlocks(start,target);}
function updateBots(dt){const level=levels[config.level];botThink-=dt;const think=botThink<=0;if(think)botThink=.13;
  for(const b of bots){
    if(b.health<=0){b.death=Math.min(1,b.death+dt*1.7);const fall=1-(1-b.death)*(1-b.death);b.group.rotation.x=-(b.deathDir??1)*fall*Math.PI/2;b.group.rotation.z=(b.deathRoll||0)*fall;b.group.position.y=-.16*fall;continue;}
    b.revealed=Math.max(0,b.revealed-dt);b.blinded=Math.max(0,(b.blinded||0)-dt);b.pathTime-=dt;b.fireTimer-=dt;const pos=b.group.position,dx=player.x-pos.x,dz=player.z-pos.z,dist=Math.hypot(dx,dz);
    const previousX=pos.x,previousZ=pos.z;
    if(think&&b.blinded<=0)b.seen=dist<37&&visibleToBot(b,dist);
    if(b.blinded>0)b.seen=false;
    if(config.mode==='competitive'&&b.nadeRole&&state.phase==='playing'&&state.elapsed>5&&b.seen&&dist>5&&dist<30){
      b.nadeTimer=Math.max(0,(b.nadeTimer??8)-dt);
    ...
    if(!b.seen||dist>16||(state.botPlant&&state.side==='CT'&&state.botPlant.bot===b)){
      ...
      b.limbs.forEach(leg=>leg.rotation.x*=.85);
      const moveSpeed=b.stats?.speed??level.speed;
      if(b.blinded<=0&&b.retreatT<=0){
        if(b.role==='sniper'&&dist<11){if(dist>.3)moveWithCollision(pos,-dx/dist*moveSpeed*.62*dt,-dz/dist*moveSpeed*.62*dt,world.obstacles,.38);}
        else if(dist<20){b.strafeT-=dt;if(b.strafeT<=0){b.strafeT=.9+Math.random()*1.1;b.strafeDir=-(b.strafeDir||1);}moveWithCollision(pos,Math.cos(b.group.rotation.y)*b.strafeDir*moveSpeed*.42*dt,-Math.sin(b.group.rotation.y)*b.strafeDir*moveSpeed*.42*dt,world.obstacles,.38);}
      }
    }
...
function frame(now){requestAnimationFrame(frame);const realDt=Math.min((now-last)/1000,.05);last=now;effects.setLowHealth(state.phase==='playing'&&player.health<=28);effects.update(realDt);state.timeScale=slowUntil>now?.28:Math.min(1,state.timeScale+realDt*1.4);document.body.classList.toggle('slowmo',state.timeScale<.9);const dt=realDt*state.timeScale;world.update?.(dt,player);clockTime+=dt;frameCounter++;fpsTime+=dt;if(fpsTime>.75){$('fps').textContent=`${Math.round(frameCounter/fpsTime)} FPS`;fpsTime=0;frameCounter=0;}
  ...
  if(state.phase==='playing'){if(config.mode!=='survival')state.time-=dt;state.elapsed+=dt;updatePlayer(dt);if(state.phase==='playing')updateBots(dt);if(state.phase==='playing')updateBomb(dt);if(state.phase==='playing')throwables.update(dt,camera);if(state.phase==='playing'){perks.update(dt,player,state,clockTime,toast,state.modifier?.id==='airdrop'?.5:1);if(config.mode==='survival')survival.update(dt);tickStreak(state.streak,clockTime);updateUpgradeFlow(dt);}if(state.time<=0&&!state.bomb&&config.mode!=='survival'&&state.phase==='playing')finish(false,'行动时间已耗尽。下次请加快推进。');updateEffects(dt);updateHUD();drawMap($('radar'),world,player,bots,state.bomb,false,perks.pickups);}
  effects.applyShake(camera);renderer.clear();renderer.render(scene,camera);if(state.phase!=='menu'&&!(state.aim&&state.weapon===2)){renderer.clearDepth();renderer.render(weaponScene,weaponCamera);}
}
...
document.addEventListener('keydown',event=>{if(state.phase!=='playing')return;if(!$('buy-menu').hidden){if(event.code==='Escape'||event.code==='KeyB'){event.preventDefault();toggleBuy(false);}return;}if(['Space','Tab','KeyW','KeyA','KeyS','KeyD','KeyR','KeyE','KeyC','KeyB','KeyQ','KeyX','KeyG','KeyV'].includes(event.code))event.preventDefault();keys.add(event.code);if(event.repeat)return;if(event.code==='KeyR')reload();if(event.code==='KeyQ')cycleWeapon();if(/^Digit[1-5]$/.test(event.code)){const slot=Number(event.code.slice(5))-1;if(upgradeOffer)chooseUpgrade(slot);else if(supplyOffer)chooseSupply(slot);else switchWeapon(slot);}if(event.code==='KeyX')cycleNade();if(event.code==='KeyG')throwSelected();if(event.code==='KeyV')melee();if(event.code==='KeyB')toggleBuy();if(event.code==='Tab')setVisible('scoreboard',true);if(event.code==='Escape')pause();});
...
$('setup').addEventListener('submit',event=>{event.preventDefault();config.mode=$('mode').value;config.modifier=$('modifier').value||'none';state.round=0;state.wins=0;state.losses=0;startRound();});
...
function refreshSkins(){guns.forEach(gun=>applySkin(gun,meta.skin));}
function renderArmory(){
  $('armory-credits').textContent=`✦ ${meta.credits}`;
  $('mastery-list').innerHTML=weaponDefinitions.map((def,index)=>{const m=meta.masteryFor(index);return `<div class="armory-row"><span>${def.name}</span><b>Lv.${m.level}</b><i><u style="width:${Math.round(m.ratio*100)}%"></u></i><small>${m.next?`${m.kills} / ${m.next}`:`${m.kills} · MAX`}</small></div>`;}).join('');
  $('daily-list').innerHTML=meta.daily().map((item,index)=>`<div class="armory-row"><span>${item.name}</span><b>${Math.min(item.progress,item.goal)}/${item.goal}</b><i><u style="width:${Math.min(100,item.progress/item.goal*100)}%"></u></i>${item.claimed?'<small>已领取</small>':item.progress>=item.goal?`<button class="mini" data-action="claim" data-index="${index}">领取 ✦${item.reward}</button>`:`<small>✦${item.reward}</small>`}</div>`).join('');
  $('skin-list').innerHTML=FINISHES.map(finish=>{const owned=meta.ownedSkins.includes(finish.id),equipped=meta.skin===finish.id,color=(finish.color??0x8a8f7a).toString(16).padStart(6,'0');return `<div class="armory-row"><span><em class="swatch" style="background:#${color}"></em>${finish.name}</span>${equipped?'<b>使用中</b>':owned?`<button class="mini" data-action="equip" data-id="${finish.id}">装备</button>`:`<button class="mini" data-action="buy" data-id="${finish.id}" ${meta.credits<finish.price?'disabled':''}>✦ ${finish.price}</button>`}</div>`;}).join('');
}
$('armory-button').addEventListener('click',()=>{renderArmory();setVisible('armory',true);});
$('armory-close').addEventListener('click',()=>setVisible('armory',false));
$('armory').addEventListener('click',event=>{const button=event.target.closest('button[data-action]');if(!button)return;const action=button.dataset.action,id=button.dataset.id,index=Number(button.dataset.index);if(action==='claim')meta.claimDaily(index);if(action==='buy')meta.buySkin(id);if(action==='equip'&&meta.equipSkin(id))refreshSkins();renderArmory();});
...
$('modifier').replaceChildren(...MODIFIERS.map(modifier=>new Option(`${modifier.name} · ${modifier.desc}`,modifier.id)));
...
if(new URLSearchParams(location.search).has('test'))window.__game={state,player,bots:()=>bots,get world(){return world;},viewModel:{guns,rig:gunRig},config,keys,shoot,reload,switchWeapon,startRound,finish,updateBomb,updateBots,pause,resume,collides,throwables,effects,cycleNade,throwSelected,melee,chooseUpgrade,chooseSupply,get upgradeOffer(){return upgradeOffer;},meta,get perks(){return perks;},survival,records,waveStats,economy,buyWeapon,buyEquip,buyNade,toggleBuy,teleport(x,z,yaw=0){Object.assign(player,{x,z,yaw,pitch:0});},aimAt(index,head=false){const p=bots[index].group.position;player.yaw=Math.atan2(-(p.x-player.x),-(p.z-player.z));player.pitch=Math.atan2((head?1.67:1.2)-(player.y+1.65),Math.hypot(p.x-player.x,p.z-player.z));camera.position.set(player.x,player.y+1.65,player.z);camera.rotation.set(player.pitch,player.yaw,0,'YXZ');camera.updateMatrixWorld(true);},aimAtPoint(x,y,z){player.yaw=Math.atan2(-(x-player.x),-(z-player.z));player.pitch=Math.atan2(y-(player.y+1.65),Math.hypot(x-player.x,z-player.z));camera.position.set(player.x,player.y+1.65,player.z);camera.rotation.set(player.pitch,player.yaw,0,'YXZ');camera.updateMatrixWorld(true);}};
